...
class ApplicationTestSettings:
    ...
    def __init__(self):
        self.application_path = os.path.abspath(os.path.dirname(introspection.__file__))

        self.package_path = os.path.normpath(
            os.path.join(
                os.path.abspath(os.path.dirname(introspection.__file__)),
                "..",
            )
        )
    ...
    def format(self, content, extra={}):
        ...
        variables = {
            ...
            "VERSION": introspection.__version__,
        }
        if extra:
